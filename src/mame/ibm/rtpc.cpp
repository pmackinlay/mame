...
void rtpc_state::common(machine_config &config)
{
	...
	m_dma[1]->out_dack_callback<0>().append(m_dma[0], FUNC(am9517a_device::hack_w));
...
void rtpc_state::ibm6150(machine_config &config)
{
	...
	// ISA slots
	ISA16_SLOT(config, m_slot[0], 0, m_isa, rtpc_isa16_cards, "fddda",    false); // slot 1: disk/diskette adapter
	ISA16_SLOT(config, m_slot[1], 0, m_isa, rtpc_isa16_cards, nullptr,    false); // slot 2: option (second fddda/esdi must be here)
	ISA8_SLOT(config,  m_slot[2], 0, m_isa, rtpc_isa8_cards,  "mda",      false); // slot 3: option (mda must be here, ega must be here or slot 6)
	...
	ISA16_SLOT(config, m_slot[7], 0, m_isa, rtpc_copro_cards, nullptr,    false); // slot 8: coprocessor/option
}
...
void rtpc_state::ibm6151(machine_config &config)
{
	...
	// ISA slots
	ISA8_SLOT(config,  m_slot[0], 0, m_isa, rtpc_isa8_cards,  "mda",      false); // slot 1: option (mda or ega must be here)
	...
	ISA16_SLOT(config, m_slot[3], 0, m_isa, rtpc_isa16_cards, nullptr,    false); // slot 4: option
	ISA16_SLOT(config, m_slot[4], 0, m_isa, rtpc_copro_cards, nullptr,    false); // slot 5: coprocessor/option
	ISA16_SLOT(config, m_slot[5], 0, m_isa, rtpc_isa16_cards, "fddda",    false); // slot 6: disk/diskette adapter
}

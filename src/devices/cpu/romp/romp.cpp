...
void romp_device::fetch(u32 address, std::function<void(u16)> f)
{
	u16 data = 0;
...
template <typename T> void romp_device::load(u32 address, std::function<void(T)> f, bool mask)
{
...
template <typename T> void romp_device::store(u32 address, T data, bool mask)
{
	rsc_mode const mode = mask ? rsc_mode((m_scr[ICS] >> 9) & 7) : rsc_mode::RSC_N;
...
template <typename T> void romp_device::modify(u32 address, std::function<T(T)> f, bool mask)
{
	rsc_mode const mode = mask ? rsc_mode((m_scr[ICS] >> 9) & 7) : rsc_mode::RSC_N;

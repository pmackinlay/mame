...
{
	set_pen_color(offset >> 1, data >> 0, data >> 8, data >> 16);
}
...
{
	set_pen_color(256 + (offset >> 1), data >> 0, data >> 8, data >> 16);
}
...
void g332_device::control_a_w(offs_t offset, u32 data, u32 mem_mask)
{
	...
	COMBINE_DATA24(&m_control_a);

	if (data & VTG_ENABLE)
	{
		LOG("display %d vdisplay %d\n", m_display, m_vdisplay); // 320 2048
		LOG("linetime %d halfsync %d backporch %d broadpulse %d\n", m_linetime, m_halfsync, m_backporch, m_broadpulse); // 434 23 56 206
		LOG("vsync %d vpreequalise %d vpostequalise %d vblank %d\n", m_vsync, m_vpreequalise, m_vpostequalise, m_vblank); // 6 2 2 62

		// frame blanking: 6 + 2 + 2 + 62 == 72 half-linetimes
		// line: 23 + 23 + 56 + 320 + ? == 434 (frontporch == 12)
		//
		// linetime: 434 (1736 pixels)
		// vblank: 36 linetimes
		// total: 1024 * 1736 + 36 * 1736 == 1060
